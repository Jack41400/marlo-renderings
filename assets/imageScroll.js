...
if (imageReel) {
    if (!imageReel.dataset.mouseDownAt) imageReel.dataset.mouseDownAt = "-18";
    if (!imageReel.dataset.prevPercentage) imageReel.dataset.prevPercentage = "-18";
}
...
window.onmouseup = () => {
    ...
    // Saves the current percentage as the previous percentage for the next drag event
...
window.onmousemove = e => {
    ...
    // stops percentage from going beyond bounds and preventing infinite scrolling
    percentage = Math.max(Math.min(percentage, -18), -82);
